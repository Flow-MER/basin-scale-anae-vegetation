...
def test_zonal_mean(mock_config):
    ...
    df = zonal_mean(ndvi_da, clear_da, tiles, int_to_uid, mock_config)
...
def test_bbox_to_wgs84(mock_config):
    ...
    with patch("data_pipelines.ndvi_landsat_monthly_dea.Geometry") as MockGeometry:
        ...
        result = bbox_to_wgs84(bbox, mock_config)
...
def test_check_dask_graph(mock_config):
    ...
    check_dask_graph(obj, mock_config, max_tasks=200)

    # Should fail if limit < 100
    with pytest.raises(RuntimeError, match="FATAL"):
        check_dask_graph(obj, mock_config, max_tasks=50)
